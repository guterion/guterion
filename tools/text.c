...
static const struct glyph *find_glyph(unsigned char c)
{
...
static int push_edge(struct edge_list *l, double x0, double y0,
		     double x1, double y1)
{
	struct edge *slot;
	...
	slot = &l->e[l->n++];
	if (y0 < y1) {
		slot->x0 = x0; slot->y0 = y0;
		slot->x1 = x1; slot->y1 = y1;
		slot->dir = 1;
	} else {
		slot->x0 = x1; slot->y0 = y1;
		slot->x1 = x0; slot->y1 = y0;
		slot->dir = -1;
	}
	return 0;
}
...
static int build_edges(struct edge_list *l, const char *s,
 ...
 * edge.
 */
static void add_span(double *row, int w, double x0, double x1, double weight)
{
	int first, last;
...
void text_draw(struct image *im, const char *s, double x, double y,
	       double size, unsigned char r, unsigned char g, unsigned char b)
{
