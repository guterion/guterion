...
static void rs_generator(int degree, unsigned char *g)
{
...
static void rs_encode(const unsigned char *data, int n, int ecc_len,
		      unsigned char *ecc)
{
	unsigned char g[64];
...
static void set_module(struct grid *g, int x, int y, int dark)
{
...
static void place_finder(struct grid *g, int ox, int oy)
{
	for (int y = -1; y <= 7; y++)
		for (int x = -1; x <= 7; x++) {
			...
			set_module(g, ox + x, oy + y,
				   inside && (ring || core));
		}
}

static void place_alignment(struct grid *g, int cx, int cy)
{
	for (int y = -2; y <= 2; y++)
		for (int x = -2; x <= 2; x++) {
...
static void place_function_patterns(struct grid *g, int version)
{
	int n = g->size;
	...
	if (VERSIONS[version].align)
		place_alignment(g, VERSIONS[version].align,
				VERSIONS[version].align);
 ...
 */
static void place_data(struct grid *g, const unsigned char *bits, int total)
{
	int n = g->size;
	int bit = 0;
	int upward = 1;
...
static void apply_mask(struct grid *g, int mask)
{
 ...
 * never all zero.
 */
static void place_format(struct grid *g, int mask)
{
	int n = g->size;
...
struct qr *qr_make(const char *text)
{
...

	{
		...
		rs_encode(codewords, total, VERSIONS[version].ecc,
			  codewords + total);
	}
