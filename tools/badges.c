 ...
 */
static const char* capitalise(
    const struct badge* b,
    char* buf,
    size_t n
)
{
...
static struct image* badge_image(
    const struct badge* b,
    const char* label,
    int width,
    int scale
)
{
    ...
    text_draw(
        im,
        label,
        (double)(PAD + MARK_COLUMN + 7) * scale,
        (double)BASELINE * scale,
        (double)TEXT_SIZE * scale,
        (unsigned char)(ink >> 16),
        (unsigned char)(ink >> 8),
        (unsigned char)ink
    );
    return im;
...
)
{
    ...
    if (text_path(glyph_path, sizeof glyph_path, label) >= sizeof glyph_path) {
