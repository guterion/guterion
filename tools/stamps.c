...
static const struct { const char *name; unsigned int rgb; } BORDERS[] = {
	{ "red", 0xFF4C49 }, { "orange", 0xE7A739 }, { "yellow", 0xC98A04 },
	{ "lime", 0x9DDB3C }, { "green", 0x44E084 }, { "blue", 0x3EA4F8 },
	{ "violet", 0xA573FF }, { "magenta", 0xD5268A },
	{ NULL, 0 }
};

/* Borders that must hold, whatever the solver would prefer. */
static const struct { const char *stamp; const char *colour; } PINNED[] = {
	{ "laroja", "red" },
	{ NULL, NULL }
};

/* Grounds named by hand, for marks whose own colour says the wrong thing. */
static const struct { const char *stamp; unsigned int rgb; } FIXED[] = {
...
static const struct cell WALL[ROWS][COLS] = {
	{
	 { "instituto-nacional", "INSTITUTO", "NACIONAL", "instituto-nacional.png", { "violet", "magenta" } },
...
static void draw_line(struct image *im, const char *text, int y,
		      unsigned int rgb)
{
...
static unsigned int ground_for(const char *name, const struct image *mark)
{
	double sum[3] = { 0, 0, 0 };
	long n = 0;
...
static int far_enough(unsigned int a, unsigned int b)
{
 ...
 */
static unsigned int shade(unsigned int rgb, int step)
{
...
static int is_fixed(const char *name)
{
	for (int i = 0; FIXED[i].stamp; i++)
		if (!strcmp(FIXED[i].stamp, name))
			return 1;
...
static int border_index(const char *name)
{
...
static void solve_borders(void)
{
...
static void separate_grounds(void)
{
	int moved = 0;
		...
		for (int c = 0; c < COLS; c++) {
			...
			for (int turn = 1; turn < 8; turn++) {
				...
				/* Alternate down and up, so the wall keeps
				   both weights. */
				ground_of[r][c] = shade(ground_of[r][c],
					((r + c) % 2) ? -turn : turn);
				if (turn == 1)
					moved++;
			}
...
static void check(void)
{
	...
	if (left)
		printf("  grounds: %d pairs still sit close; name one in FIXED\n",
		       left / 2);
	else
		printf("  grounds: every touching pair separated\n");
}

static void write_stamp(const struct cell *cell, unsigned int accent,
			unsigned int ground)
{
...
int main(void)
{
	...
	for (int r = 0; r < ROWS; r++)
		for (int c = 0; c < COLS; c++) {
			...
			snprintf(path, sizeof path, "assets/logos/%s",
				 WALL[r][c].logo);
			mark = png_read(path);
			if (!mark) {
				fprintf(stderr, "cannot read %s\n", path);
	...
	for (int r = 0; r < ROWS; r++) {
		printf(" ");
		for (int c = 0; c < COLS; c++)
			printf(" %9.9s %-4.4s#%06x", WALL[r][c].name,
			       BORDERS[border_of[r][c]].name, ground_of[r][c]);
		printf("\n");
	}
	return 0;
}
