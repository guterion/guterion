...
)
{
    ...
    if (n < 0 || (size_t)n >= sizeof path) {
        report(
            "%s: the path to %s needs more than %d bytes",
            doc,
            target,
            MAX_PATH
        );
 ...
 */
static void check_quoted_paths(
    const char* doc,
    const char* text
)
{
    static const char* const roots[] = {"assets/", "tools/", "i18n/", NULL};
    const char* p = text;
...
)
{
    ...
    printf(
        "All boxes aligned, every link and quoted path resolves, "
